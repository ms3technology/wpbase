...
/*=======================
Sync Optional Subjects
========================*/
if (isset($_POST['syncOptionalSubjects'])) {
        $class_id = intval($_POST['syncClass']);
        $group_id = intval($_POST['syncGroup']);

        // Fetch optional subject IDs for this class + group
        $subSql = "SELECT subjectid FROM ct_subject WHERE subOptinal = 1 AND subjectClass = $class_id";
        // forGroup can be 'all', a numeric ID, or a JSON array
        $subSql .= " AND (forGroup = 'all' OR forGroup = '$group_id'";
        $subSql .= " OR forGroup LIKE '%\"$group_id\"%')";
        $optional_subjects = $wpdb->get_col($subSql);

        if (empty($optional_subjects)) {
                $message = ms3message(false, 'No optional subjects found for this class/group.');
        } else {
                $json_ids = json_encode(array_map('strval', $optional_subjects));

                // Find studentinfo rows with matching class+group AND empty infoOptionals
                $updated = $wpdb->query($wpdb->prepare(
                        "UPDATE ct_studentinfo SET infoOptionals = %s WHERE infoClass = %d AND infoGroup = %d AND (infoOptionals IS NULL OR infoOptionals = '' OR infoOptionals = '0')",
                        $json_ids, $class_id, $group_id
                ));

                $count = is_numeric($updated) ? $updated : 0;
                $message = ms3message($updated !== false, $count . ' student(s) updated with optional subjects (' . count($optional_subjects) . ' subjects).');
        }
}
...
<div class="container-fluid maxAdminpages" style="padding-left: 0">
  ...
  <h2>Subject Management
    ...
    <button class="pull-right btn btn-info" style="margin-right: 10px;" id="syncOptionalBtn">
      <span class="dashicons dashicons-update"></span> Sync Optional Subjects
    </button>
  </h2>
  <br>
...
<!--========================
	Sync Optional Subjects Modal
=========================-->
<div id="syncOptionalModal" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Sync Optional Subjects</h4>
      </div>
      <div class="modal-body">
        <p>Find students in a class+group who have no optional subjects assigned, and fill them automatically.</p>
        <form id="syncOptionalForm">
          <div class="form-group">
            <label>Class</label>
            <select id="syncClass" class="form-control" required>
              <option value="">Select Class</option>
              <?php
              $all_classes = $wpdb->get_results("SELECT * FROM ct_class ORDER BY className");
          ...
          <p><strong>Note:</strong> All groups for the selected class will be processed automatically one by one.</p>
          <div id="syncOptionalResult" style="display:none;" class="alert"></div>
          <div id="syncOptionalProgress" style="display:none;">
            <hr>
            <h5>Progress:</h5>
            <ul id="syncOptionalProgressList" style="padding-left:20px;"></ul>
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-success" id="syncOptionalSubmit">
...
<script type="text/javascript">
  ...
  if ($('.religionck:checked').length > 0) {
    ...
    // ===== Sync Optional Subjects =====
    /*Open Sync Modal*/
    $('#syncOptionalBtn').click(function() {
      $('#syncOptionalResult').hide().removeClass('alert-success alert-danger alert-warning').html('');
      $('#syncOptionalProgress').hide();
      $('#syncOptionalProgressList').empty();
      $('#syncClass').val('');
      $('#syncOptionalSubmit').prop('disabled', false).html('<span class="dashicons dashicons-update"></span> Sync Now');
      $('#syncOptionalModal').modal('show');
    });

    /*Submit Sync — processes all groups for the selected class*/
    $('#syncOptionalSubmit').click(function() {
      var classId = $('#syncClass').val();
      var $btn = $(this);
      var $result = $('#syncOptionalResult');
      var $progress = $('#syncOptionalProgress');
      var $progressList = $('#syncOptionalProgressList');

      if (!classId) {
        $result.removeClass('alert-success alert-danger').addClass('alert-warning').html('Please select a Class.').show();
        return;
      }

      $btn.prop('disabled', true).html('<span class="dashicons dashicons-update" style="animation: spin 1s linear infinite;"></span> Syncing...');
      $result.hide().removeClass('alert-success alert-danger alert-warning').html('');
      $progress.show();
      $progressList.append('<li><em>Looking up groups for this class...</em></li>');
        ...
        success: function(response) {
          ...
          if (response.success) {
            ...
            if (response.data.results && response.data.results.length) {
              $.each(response.data.results, function(i, line) {
                $progressList.append('<li>' + line + '</li>');
              });
            }
            $result.removeClass('alert-danger alert-warning').addClass('alert-success').html(response.data.summary).show();
          } else {
            $result.removeClass('alert-success alert-warning').addClass('alert-danger').html(response.data).show();
          }
        },
        error: function() {
          $result.removeClass('alert-success alert-warning').addClass('alert-danger').html('Network error. Please try again.').show();
          $progressList.append('<li class="text-danger">Error — sync failed.</li>');
        },
        complete: function() {
