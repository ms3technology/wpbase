...
if (wp_get_current_user()->roles[0] == 'um_headmaster' || current_user_can('administrator')) {
    ...
    // AJAX handler for theme image upload (direct file upload like student-defalt)
    if (isset($_POST['type']) && $_POST['type'] === 'uploadThemeImage') {
        @ini_set('memory_limit', '512M');
        @set_time_limit(300);

        if (!isset($_FILES['theme_image']) || !isset($_POST['field_name'])) {
            wp_send_json_error('Invalid request');
        }

        $field_name = sanitize_text_field($_POST['field_name']);
        $uploadedfile = $_FILES['theme_image'];
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if (!is_wp_error($attach_id)) {
                ...
                // Disable thumbnail generation to save memory
                ...
                if ($table_exists === $sm_table) {
                    $wpdb->replace(
                        $sm_table,
                        ['option_name' => $field_name, 'option_value' => $uploaded_image_url, 'autoload' => 'yes'],
                        ['%s', '%s', '%s']
                    );
                }

                wp_send_json_success(['url' => $uploaded_image_url, 'id' => $attach_id]);
            } else {
    ...
    // AJAX handler for slider image upload (direct file upload)
    if (isset($_POST['type']) && $_POST['type'] === 'uploadSliderImage') {
        @ini_set('memory_limit', '512M');
        @set_time_limit(300);
        ...
        $upload_overrides = ['test_form' => false];
        $movefile = wp_handle_upload($_FILES['slider_image'], $upload_overrides);
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if ($saved) {
                wp_send_json_success(['url' => $image_url, 'id' => $wpdb->insert_id]);
            } else {
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div id="tab1" class="tab-content">
                        ...
                        <table class="form-table" role="presentation" style="color:#000">
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">
                                        <div class="redux_field_th">Principal Sign</div>
                                    </th>
                                    <td>
                                        <div class="theme-img-upload-container" data-field="principalSign">
                                            <img src="<?php echo esc_url(get_image_url($redux_options['principalSign']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/anwar-sir.png'); ?>" class="theme-img-preview" id="preview_principalSign">
                                            <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                            <input type="file" class="theme-img-input" accept="image/*">
                                            <input type="hidden" class="theme-img-url" name="opt_name[principalSign][url]" value="<?php echo esc_attr($redux_options['principalSign']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/anwar-sir.png'); ?>">
                                            <input type="hidden" class="theme-img-id" name="opt_name[principalSign][id]" value="<?php echo esc_attr($redux_options['principalSign']['id'] ?? '4082'); ?>">
                                            <button type="button" class="theme-img-remove button" data-field="principalSign">Remove</button>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">
                                        <div class="redux_field_th">Barcode Or Logo for ID card</div>
                                    </th>
                                    <td>
                                        <div class="theme-img-upload-container" data-field="barcode">
                                            <img src="<?php echo esc_url(get_image_url($redux_options['barcode']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_barcode">
                                            <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                            <input type="file" class="theme-img-input" accept="image/*">
                                            <input type="hidden" class="theme-img-url" name="opt_name[barcode][url]" value="<?php echo esc_attr($redux_options['barcode']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                                            <input type="hidden" class="theme-img-id" name="opt_name[barcode][id]" value="<?php echo esc_attr($redux_options['barcode']['id'] ?? '4083'); ?>">
                                            <button type="button" class="theme-img-remove button" data-field="barcode">Remove</button>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">
                                        <div class="redux_field_th">Institute logo for Admit/Seat card</div>
                                    </th>
                                    <td>
                                        <div class="theme-img-upload-container" data-field="instLogo">
                                            <img src="<?php echo esc_url(get_image_url($redux_options['instLogo']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_instLogo">
                                            <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                            <input type="file" class="theme-img-input" accept="image/*">
                                            <input type="hidden" class="theme-img-url" name="opt_name[instLogo][url]" value="<?php echo esc_attr($redux_options['instLogo']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                                            <input type="hidden" class="theme-img-id" name="opt_name[instLogo][id]" value="<?php echo esc_attr($redux_options['instLogo']['id'] ?? ''); ?>">
                                            <button type="button" class="theme-img-remove button" data-field="instLogo">Remove</button>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                    ...
                    <div id="homeslider" class="tab-content active">
                        ...
                        <div class="wrap">
                            ...
                            <p class="description" style="color: #d63638; font-weight: 600;">⚠️ Click the image area to upload. Max 2MB.</p>
                            
                            <!-- Clickable upload container like student-defalt -->
                            <div class="slider-upload-container" id="sliderUploadContainer">
                                <div class="slider-upload-inner">
                                    <span class="dashicons dashicons-plus-alt" style="font-size: 48px; color: #aaa;"></span>
                                    <p style="color: #888; margin: 10px 0 0 0;">Click to upload slider image</p>
                                </div>
                                <input type="file" id="sliderImageInput" accept="image/*">
                            </div>
                            <div id="sliderUploadProgress" style="display:none; margin-top: 10px; color: #2271b1; font-weight: 600;">
                                <span class="dashicons dashicons-update" style="animation: spin 1s linear infinite;"></span> Uploading...
                            </div>
                        ...
                        <style>
                            ...
                            .slider-uploading #sliderImageInput {
                                pointer-events: none;
                            }
                            .slider-upload-container:hover {
                                border-color: #2271b1;
                                background: #f0f6fc;
                            }
                            .slider-upload-container.slider-uploading {
                                opacity: 0.5;
                                pointer-events: none;
                            }
                            .slider-upload-container.slider-uploading::after {
                                content: "";
                                position: absolute;
                                width: 30px;
                                height: 30px;
                            ...
                            .theme-img-upload-container:hover {
                                border-color: #2271b1;
                                background: #f0f6fc;
                            }
                            .theme-img-upload-container img {
                                max-width: 100%;
                                max-height: 100%;
                                object-fit: contain;
                            }
                            .theme-img-upload-overlay {
                                position: absolute;
                                top: 0;
                                left: 0;
                                width: 100%;
                            ...
                            .theme-img-upload-container:hover .theme-img-upload-overlay {
                                display: flex;
                            }
                            .theme-img-uploading {
                                opacity: 0.5;
                            ...
                            .theme-img-uploading .theme-img-input {
                                pointer-events: none;
                            }
                            .theme-img-remove {
                                position: absolute;
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                ...
                                $('#sliderImageInput').on('change', function(e) {
                                    ...
                                    var container = $('#sliderUploadContainer');
                                    var formData = new FormData();
                                    formData.append('slider_image', file);
                                    formData.append('type', 'uploadSliderImage');

                                    container.addClass('slider-uploading');
                                    $('#sliderUploadProgress').show();
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            if (response.success) {
                                                ...
                                                container.removeClass('slider-uploading');
                                                $('#sliderUploadProgress').hide();
                                            }
                                        },
                                        error: function() {
                                            container.removeClass('slider-uploading');
                                ...
                                // Theme image upload - handle file selection (input directly clickable via opacity:0)
                                $(document).on('change', '.theme-img-input', function(e) {
                                    var file = e.target.files[0];
                                    if (!file) return;

                                    var container = $(this).closest('.theme-img-upload-container');
                                    var fieldName = container.data('field');
                                    var formData = new FormData();
                                    formData.append('theme_image', file);
                                    formData.append('field_name', fieldName);
                                    formData.append('type', 'uploadThemeImage');

                                    container.addClass('theme-img-uploading');
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(resp) {
                                            container.removeClass('theme-img-uploading');
                                            if (resp.success) {
                                                container.find('.theme-img-preview').attr('src', resp.data.url);
                                                container.find('.theme-img-url').val(resp.data.url);
                                                container.find('.theme-img-id').val(resp.data.id);
                                            } else {
                                ...
                                // Theme image upload - remove image
                                $(document).on('click', '.theme-img-remove', function(e) {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    if (!confirm('Remove this image?')) return;
                                    var container = $(this).closest('.theme-img-upload-container');
                                    var fieldName = container.data('field');
                                    var placeholder = '<?php echo get_site_url(); ?>/wp-content/uploads/2024/01/logo.png';
                                    container.find('.theme-img-url').val('');
                                    container.find('.theme-img-id').val('');
                                    container.find('.theme-img-preview').attr('src', placeholder);
                                });
                            });
                        </script>
                        ...
                        <div style="width: 100%; display: flex; gap: 30px; margin-top: 20px;">
                            <?php
                            // Helper function for pad fields (direct file upload)
                            $render_pad_field = function($id, $label, $redux_options) {
                                $pad_value = $redux_options[$id] ?? '';
                                $pad_url = $pad_value ? get_image_url($pad_value) : '';
                                ?>
                                <div class="testimonial-settings">
                                    <label for="opt_name_<?php echo esc_attr($id); ?>"><?php echo esc_html($label); ?></label>
                                    <div class="theme-img-upload-container" data-field="<?php echo esc_attr($id); ?>">
                                        <?php if ($pad_url) : ?>
                                            <img src="<?php echo esc_url($pad_url); ?>" class="theme-img-preview" id="preview_<?php echo esc_attr($id); ?>">
                                        <?php else : ?>
                                            <img src="" class="theme-img-preview" id="preview_<?php echo esc_attr($id); ?>" style="display:none;">
                                            <span style="color:#999;font-size:12px;">Click to upload</span>
                                        <?php endif; ?>
                                        <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                        <input type="file" class="theme-img-input" accept="image/*,.pdf">
                                        <input type="hidden" class="theme-img-url" name="opt_name[<?php echo esc_attr($id); ?>]" value="<?php echo esc_attr($pad_value); ?>">
                                        <button type="button" class="theme-img-remove button" data-field="<?php echo esc_attr($id); ?>">Remove</button>
                                    </div>
                                </div>
                                <?php
                            };
                    ...
                    <div id="header" class="tab-content">
                        ...
                        <div class="theme-img-upload-container" data-field="logo_upload">
                            <img src="<?php echo esc_url(get_image_url($redux_options['logo_upload']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_logo_upload">
                            <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                            <input type="file" class="theme-img-input" accept="image/*">
                            <input type="hidden" class="theme-img-url" name="opt_name[logo_upload][url]" value="<?php echo esc_attr($redux_options['logo_upload']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                            <input type="hidden" class="theme-img-id" name="opt_name[logo_upload][id]" value="<?php echo esc_attr($redux_options['logo_upload']['id'] ?? ''); ?>">
                            <button type="button" class="theme-img-remove button" data-field="logo_upload">Remove</button>
                        </div>
                    ...
                    <div id="aboutus" class="tab-content">
                        ...
                        <div class="form-container">
                            <div class="form-group">
                                ...
                                <div class="theme-img-upload-container" data-field="home_about_img">
                                    <img src="<?php echo esc_url(get_image_url($redux_options['home_about_img']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_home_about_img">
                                    <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                    <input type="file" class="theme-img-input" accept="image/*">
                                    <input type="hidden" class="theme-img-url" name="opt_name[home_about_img][url]" value="<?php echo esc_attr($redux_options['home_about_img']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                                    <input type="hidden" class="theme-img-id" name="opt_name[home_about_img][id]" value="<?php echo esc_attr($redux_options['home_about_img']['id'] ?? ''); ?>">
                                    <button type="button" class="theme-img-remove button" data-field="home_about_img">Remove</button>
                                </div>
                            </div>
                            <div class="form-group">
                    ...
                    <div id="headmaster" class="tab-content">
                        ...
                        <div class="form-container">
                            ...
                            <div class="form-group">
                                ...
                                <div class="theme-img-upload-container" data-field="homeHeadmasterImg">
                                    <img src="<?php echo esc_url(get_image_url($redux_options['homeHeadmasterImg']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_homeHeadmasterImg">
                                    <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                    <input type="file" class="theme-img-input" accept="image/*">
                                    <input type="hidden" class="theme-img-url" name="opt_name[homeHeadmasterImg][url]" value="<?php echo esc_attr($redux_options['homeHeadmasterImg']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                                    <input type="hidden" class="theme-img-id" name="opt_name[homeHeadmasterImg][id]" value="<?php echo esc_attr($redux_options['homeHeadmasterImg']['id'] ?? ''); ?>">
                                    <button type="button" class="theme-img-remove button" data-field="homeHeadmasterImg">Remove</button>
                                </div>
                            </div>
                            <div class="form-group">
                    ...
                    <div id="chairman" class="tab-content">
                        ...
                        <div class="form-container">
                            <div class="form-group">
                                <label>Upload Chairman Photo</label>
                                <div class="theme-img-upload-container" data-field="homeChairmanImg">
                                    <img src="<?php echo esc_url(get_image_url($redux_options['homeChairmanImg']['url'] ?? '') ?: get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>" class="theme-img-preview" id="preview_homeChairmanImg">
                                    <div class="theme-img-upload-overlay"><span class="dashicons dashicons-camera"></span></div>
                                    <input type="file" class="theme-img-input" accept="image/*">
                                    <input type="hidden" class="theme-img-url" name="opt_name[homeChairmanImg][url]" value="<?php echo esc_attr($redux_options['homeChairmanImg']['url'] ?? get_site_url() . '/wp-content/uploads/2024/01/logo.png'); ?>">
                                    <input type="hidden" class="theme-img-id" name="opt_name[homeChairmanImg][id]" value="<?php echo esc_attr($redux_options['homeChairmanImg']['id'] ?? ''); ?>">
                                    <button type="button" class="theme-img-remove button" data-field="homeChairmanImg">Remove</button>
                                </div>
                            </div>
